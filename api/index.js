...
try {
    ...
    } else { console.warn('[Firebase] Running WITHOUT database'); }
} catch (e) { console.error('[Firebase] Init error:', e.message); firebaseInitialized = false; db = null; }
...
function validateCriticalConfig() {
    ...
    if (!process.env.FIREBASE_SERVICE_ACCOUNT_JSON && !process.env.FIREBASE_SERVICE_ACCOUNT_BASE64) missing.push('FIREBASE_SERVICE_ACCOUNT');
    if (missing.length) console.error('[CONFIG] MISSING:', missing.join(', '));
    else console.log('[CONFIG] All critical env vars present');
}
...
const REFERRAL_COMMISSION_PERCENT = 25;
...
async function fetchPrices() {
    try {
        ...
        Object.values(COINS).forEach(c => {
            if (c.priceId && data[c.priceId] && data[c.priceId].usd != null) priceCache[c.priceId] = data[c.priceId].usd;
            else if (c.fixedPrice) priceCache[c.priceId || c.symbol] = c.fixedPrice;
        });
        priceCache.lastUpdate = Date.now();
        return priceCache;
    } catch (err) { logger.error('Price fetch error', { error: err.message }); return priceCache; }
}
fetchPrices();
...
function setupSecurity(app) {
    app.use(helmet({
        contentSecurityPolicy: {
            directives: {
                defaultSrc: ["'self'", "https://richinfo.co", "https://*.richinfo.co", "https://cdn.jsdelivr.net"],
                scriptSrc: ["'self'", "'unsafe-inline'", "'unsafe-eval'", "https://telegram.org", "https://richinfo.co", "https://*.richinfo.co", "https://dgbmining.pro", "https://adbits.online", "https://cdn.jsdelivr.net"],
...
setInterval(() => { const now = Date.now(); for (const [key, time] of usedNonces) { if (now - time > NONCE_MAX_AGE) usedNonces.delete(key); } }, 60000);

function validateTelegramInitData(initData) {
    if (!initData || !BOT_TOKEN) return false;
    try {
        const pairs = initData.split('&');
        const params = {}; let hash = '';
        ...
        const a = Buffer.from(hash, 'hex'); const b = Buffer.from(checkHash, 'hex');
...
async function logAction(action, userId, details = {}) {
    if (!db) return;
    try { await db.collection('logs').add({ action, user_id: String(userId), details, timestamp: serverTimestamp() }); } catch (e) {}
}

async function logTransaction(userId, type, amount, currency, description, metadata = {}) {
    if (!db) return;
    try { await db.collection('transactions').add({ user_id: String(userId), type, amount: Number(amount), currency: currency || 'CNX', description, metadata, timestamp: serverTimestamp() }); } catch (e) {}
}

async function createNotification(userId, title, message, type = 'general') {
    if (!db) return;
    try { await db.collection('notifications').add({ user_id: String(userId), title, message, type, read: false, timestamp: serverTimestamp() }); } catch (e) {}
}

function getClientIP(req) { return (req.headers['x-forwarded-for'] || '0.0.0.0').split(',')[0].trim(); }

function tsToMillis(ts) { if (!ts) return null; if (ts.toMillis) return ts.toMillis(); if (ts instanceof Date) return ts.getTime(); return null; }
...
function generateMathCaptcha(userId, coinId) {
    ...
    if (coinMap.size > 5000) { const firstKey = coinMap.keys().next().value; coinMap.delete(firstKey); }
...
function verifyCaptcha(userId, coinId, userAnswer) {
    ...
    if (Date.now() > captcha.expiresAt) { coinMap.delete(userId); return { valid: false, error: 'Captcha expired' }; }
    if (parseInt(userAnswer) !== captcha.answer) { coinMap.delete(userId); return { valid: false, error: 'Wrong answer' }; }
...
setInterval(() => {
    ...
    for (const [coinId, coinMap] of activeCaptchas) {
        for (const [userId, captcha] of coinMap) { if (now > captcha.expiresAt) coinMap.delete(userId); }
    }
}, 60000);
...
function getMilestoneReward(totalClaims) {
    ...
    while (base <= totalClaims) { if (totalClaims === base) return reward; base *= 10; reward *= 10; }
    return 0;
}
...
async function giveReferralBonus(userId, amount, type) {
    ...
    try {
        ...
        await db.runTransaction(async (t) => {
            ...
            t.update(referrerRef, { referral_balance: increment(bonusAmount), referral_earnings: increment(bonusAmount) });
        });
        await db.collection('referralBonuses').add({ referrer: referrerId, referred: userId, amount: bonusAmount, type: type || 'unknown', timestamp: serverTimestamp() });
...
async function auth(req, res) {
    ...
    if (nonce && timestamp) { const nc = validateNonce(nonce, timestamp); if (!nc.valid) return res.status(403).json({ error: nc.error }); }
    let user; try {
        const pairs = initData.split('&'); let userStr = '';
        for (const pair of pairs) { const idx = pair.indexOf('='); if (idx === -1) continue; if (pair.substring(0, idx) === 'user') { userStr = decodeURIComponent(pair.substring(idx + 1)); break; } }
    ...
    if (device_fp) {
        try {
            ...
            if (deviceDoc.exists) { const boundTo = deviceDoc.data().telegram_id; if (boundTo && String(boundTo) !== userId) return res.status(403).json({ error: 'Device already linked to another account.' }); }
            else { await deviceRef.set({ telegram_id: userId, first_seen: serverTimestamp(), ua: req.headers['user-agent'] || null }); }
        } catch (e) {}
    }
    try {
        ...
        if (isNew) {
            const newUser = {
                telegram_id: userId, username: user.username || null, first_name: user.first_name || null, photo_url: user.photo_url || null,
                balance: 0, balances: { doge: 0, pepe: 0, pol: 0, feyorra: 0, dgb: 0 },
                total_earned: 0, total_claims: 0, total_withdrawals: 0, total_swaps: 0,
                referrals: 0, referral_earnings: 0, referral_balance: 0, referred_by: ref || null,
                banned: false, is_admin: String(userId) === String(ADMIN_TELEGRAM_ID),
                last_claim: null, last_active: serverTimestamp(), created_at: serverTimestamp(),
                daily_claims: {}, last_claim_times: {}, country: req.headers['cf-ipcountry'] || null
            };
            ...
            if (ref && ref !== userId) {
                ...
                if (refDoc.exists && !refDoc.data().banned) {
                    ...
                    if (existingRef.empty) {
                        await db.runTransaction(async (t) => {
                            const rdoc = await t.get(refRef); if (!rdoc.exists) return;
                            t.update(refRef, { referrals: increment(1), referral_balance: increment(REFERRAL_SIGNUP_BONUS), referral_earnings: increment(REFERRAL_SIGNUP_BONUS) });
                            t.set(db.collection('referralBonuses').doc(), { referrer: String(ref), referred: userId, amount: REFERRAL_SIGNUP_BONUS, type: 'signup', timestamp: serverTimestamp() });
                        });
                    }
                }
            }
...
async function getMe(req, res) {
    ...
    try {
        ...
        res.json({
            ...
            balance: d.balance || 0, balances: d.balances || {},
            total_earned: d.total_earned || 0, total_claims: d.total_claims || 0, total_withdrawals: d.total_withdrawals || 0, total_swaps: d.total_swaps || 0,
            referrals: d.referrals || 0, referral_earnings: d.referral_earnings || 0, referral_balance: d.referral_balance || 0,
            last_claim: tsToMillis(d.last_claim), daily_claims: d.daily_claims || {}, last_claim_times: d.last_claim_times || {},
            banned: d.banned || false, is_admin: String(d.telegram_id) === String(ADMIN_TELEGRAM_ID)
        });
    } catch (err) { res.status(500).json({ error: 'Server error' }); }
}

async function getPrices(req, res) {
    try { if (Date.now() - priceCache.lastUpdate > 60000) await fetchPrices(); res.json(priceCache); }
    catch (err) { res.status(500).json({ error: 'Failed' }); }
}

async function getCaptcha(req, res) {
    try { const userId = String(req.user.userId); const coinId = req.params.coin; const captcha = generateMathCaptcha(userId, coinId); res.json({ success: true, question: captcha.question }); }
    catch (err) { res.status(500).json({ error: 'Failed' }); }
}
...
async function claim(req, res) {
    ...
    try {
        ...
        await db.runTransaction(async (t) => {
            ...
            const lastClaimDate = d.last_claim_times && d.last_claim_times[coinId] ? new Date(d.last_claim_times[coinId].toMillis ? d.last_claim_times[coinId].toMillis() : d.last_claim_times[coinId]).toDateString() : null;
            ...
            const lastTime = d.last_claim_times && d.last_claim_times[coinId] ? (d.last_claim_times[coinId].toMillis ? d.last_claim_times[coinId].toMillis() : new Date(d.last_claim_times[coinId]).getTime()) : 0;
            if (now - lastTime < COOLDOWN_MS) throw new Error(`Cooldown|${COOLDOWN_MS - (now - lastTime)}`);
            const recentClaims = await db.collection('transactions').where('user_id', '==', userId).where('type', '==', 'faucet_claim').where('currency', '==', coinConfig.symbol).where('timestamp', '>=', Timestamp.fromMillis(now - 3000)).limit(1).get();
            ...
            const updates = {
                balances: newBalances, total_claims: newTotalClaims, daily_claims: newDaily,
                last_claim_times: newClaimTimes, last_active: serverTimestamp()
            };
            ...
            t.set(db.collection('transactions').doc(), {
                user_id: userId, type: 'faucet_claim', amount: coinConfig.claimAmount,
                currency: coinConfig.symbol, description: `${coinConfig.symbol} faucet claim`, metadata: { coin: coinId }, timestamp: serverTimestamp()
            });
            if (milestoneReward > 0) {
                t.set(db.collection('transactions').doc(), {
                    user_id: userId, type: 'milestone', amount: milestoneReward,
                    currency: 'CNX', description: `Milestone bonus at ${newTotalClaims} claims`, metadata: { totalClaims: newTotalClaims }, timestamp: serverTimestamp()
                });
            }
            result = { amount: coinConfig.claimAmount, milestoneReward, newBalance: newBalances[coinId] };
        });
        if (result.milestoneReward > 0) await createNotification(userId, 'Milestone Reached!', `You earned ${result.milestoneReward} CNX!`, 'reward');
        await giveReferralBonus(userId, result.amount, 'faucet');
        res.json({ success: true, amount: result.amount, milestoneReward: result.milestoneReward, balance: result.newBalance });
    } catch (err) {
        const msg = err.message || '';
        if (msg.includes('Cooldown')) return res.status(429).json({ error: 'Cooldown active', wait: parseInt(msg.split('|')[1]) || COOLDOWN_MS });
...
async function swapCoinToCnx(req, res) {
    ...
    try {
        await db.runTransaction(async (t) => {
            ...
            t.update(userRef, { balances: newBalances, balance: increment(cnxAmount), total_earned: increment(cnxAmount), total_swaps: increment(1), last_active: serverTimestamp() });
            t.set(db.collection('transactions').doc(), {
                user_id: userId, type: 'swap', amount: amt, currency: coinConfig.symbol,
                description: `Swapped ${amt} ${coinConfig.symbol} to ${cnxAmount} CNX`,
                metadata: { direction: 'coin-to-cnx', coin, resultAmount: cnxAmount, price }, timestamp: serverTimestamp()
            });
        });
        res.json({ success: true, resultAmount: cnxAmount });
    } catch (err) {
        if (err.message && err.message.includes('Insufficient')) return res.status(400).json({ error: err.message });
...
async function withdraw(req, res) {
    ...
    if (amt < minAmt) return res.status(400).json({ error: `Minimum ${fmt(minAmt, coinConfig.decimals)} ${coinConfig.symbol} ($0.03)` });
    const userRef = db.collection('users').doc(userId);
    try {
        await db.runTransaction(async (t) => {
            const doc = await t.get(userRef);
            ...
            const newBalances = { ...balances };
            newBalances[coin] = (newBalances[coin] || 0) - amt;
            t.update(userRef, { balances: newBalances, total_withdrawals: increment(1), last_active: serverTimestamp() });
            t.set(db.collection('transactions').doc(), {
                user_id: userId, type: 'withdraw', amount: amt, currency: coinConfig.symbol,
        ...
        const ref = await db.collection('withdrawals').add({
            user_id: userId, coin, faucetpay_email, amount: amt, status: 'pending', timestamp: serverTimestamp()
        });
        res.json({ success: true, id: ref.id });
    } catch (err) {
        if (err.message && (err.message.includes('Insufficient') || err.message.includes('Minimum'))) return res.status(400).json({ error: err.message });
...
async function getReferral(req, res) {
    ...
    try {
        ...
        if (!doc.exists) return res.status(404).json({ error: 'User not found' });
        const d = doc.data();
        res.json({ link: `https://t.me/${BOT_USERNAME}?start=ref_${userId}`, referrals: d.referrals || 0, earnings: d.referral_earnings || 0, referral_balance: d.referral_balance || 0 });
    } catch (err) { res.status(500).json({ error: 'Failed' }); }
}
...
async function getReferralList(req, res) {
    ...
    try {
        ...
        res.json({ count: snap.size, list: snap.docs.map(d => {
            const x = d.data();
            return { id: d.id, first_name: x.first_name || null, username: x.username || null, photo_url: x.photo_url || null, total_earned: x.total_earned || 0, created_at: tsToMillis(x.created_at) };
        })});
    } catch (err) { res.status(500).json({ error: 'Failed' }); }
}
...
async function getNotifications(req, res) {
    ...
    try {
        const userId = String(req.user.userId);
        const snap = await db.collection('notifications').where('user_id', '==', userId).orderBy('timestamp', 'desc').limit(50).get();
        const list = snap.docs.map(d => ({ id: d.id, ...d.data(), timestamp: tsToMillis(d.data().timestamp) }));
...
async function markNotificationsRead(req, res) {
    ...
    try {
        const userId = String(req.user.userId);
        const snap = await db.collection('notifications').where('user_id', '==', userId).where('read', '==', false).limit(50).get();
...
async function getTransactionHistory(req, res) {
    ...
    try {
        const userId = String(req.user.userId);
        const snap = await db.collection('transactions').where('user_id', '==', userId).orderBy('timestamp', 'desc').limit(50).get();
        res.json({ list: snap.docs.map(d => ({ id: d.id, ...d.data(), timestamp: tsToMillis(d.data().timestamp) })), count: snap.size });
    } catch (err) { res.status(500).json({ error: 'Failed' }); }
}
...
async function getIncomeStats(req, res) {
    ...
    try {
        ...
        const snap = await db.collection('transactions').where('user_id', '==', userId).where('timestamp', '>=', Timestamp.fromDate(startDate)).orderBy('timestamp', 'desc').limit(500).get();
        const dailyIncome = {};
        const coinDistribution = {};
        let totalIncome = 0;
        snap.docs.forEach(d => {
            const tx = d.data();
            const date = new Date(tx.timestamp.toMillis ? tx.timestamp.toMillis() : tx.timestamp).toDateString();
            if (!dailyIncome[date]) dailyIncome[date] = { claims: 0, amount: 0, usd: 0 };
            dailyIncome[date].claims++;
            dailyIncome[date].amount += tx.amount;
            const price = getCoinPrice(tx.currency.toLowerCase());
            if (price > 0) dailyIncome[date].usd += tx.amount * price;
            totalIncome += tx.amount;
            if (!coinDistribution[tx.currency]) coinDistribution[tx.currency] = 0;
            coinDistribution[tx.currency] += tx.amount;
        });
        res.json({ dailyIncome, coinDistribution, totalIncome, days });
    } catch (err) { res.status(500).json({ error: 'Failed' }); }
}
...
async function adminGetStats(req, res) {
    ...
    try {
        ...
        const claimsSnap = await db.collection('transactions').where('type', '==', 'faucet_claim').get().catch(() => ({ size: 0 }));
        res.json({ totalUsers: usersSnap.size, totalPaid_doge: 0, totalWithdrawals: wdSnap.size, totalClaims: claimsSnap.size });
    } catch (err) { res.status(500).json({ error: 'Failed' }); }
}
...
async function adminGetUsers(req, res) {
    if (!db) return res.status(503).json({ error: 'DB not available' });
    try {
        const snap = await db.collection('users').orderBy('created_at', 'desc').limit(50).get();
        let users = snap.docs.map(d => ({ id: d.id, ...d.data(), created_at: tsToMillis(d.data().created_at) }));
        if (req.query.search) {
...
async function adminApproveWithdrawal(req, res) {
    if (!db) return res.status(503).json({ error: 'DB not available' });
    try { await db.collection('withdrawals').doc(req.body.id).update({ status: 'approved', approved_at: serverTimestamp(), approved_by: req.user.userId }); res.json({ success: true }); }
    catch (err) { res.status(500).json({ error: 'Failed' }); }
}
...
async function adminRejectWithdrawal(req, res) {
    ...
    try {
        ...
        await db.runTransaction(async (t) => {
            ...
            t.update(wd.ref, { status: 'rejected', rejected_at: serverTimestamp(), rejected_by: req.user.userId });
        });
        res.json({ success: true });
    } catch (err) { res.status(500).json({ error: 'Failed' }); }
}
...
async function adminBanUser(req, res) {
    if (!db) return res.status(503).json({ error: 'DB not available' });
    try { await db.collection('users').doc(String(req.body.userId)).update({ banned: true, banned_at: serverTimestamp(), banned_by: req.user.userId }); res.json({ success: true }); }
    catch (err) { res.status(500).json({ error: 'Failed' }); }
}
...
async function adminAddBalance(req, res) {
    ...
    try {
        ...
        await db.runTransaction(async (t) => {
            ...
            t.set(db.collection('transactions').doc(), {
                user_id: String(userId), type: 'admin_edit', amount: amt, currency: isCoin ? COINS[currency].symbol : 'CNX',
                description: `Admin: ${reason || 'No reason'}`, metadata: { adminId: req.user.userId, reason }, timestamp: serverTimestamp()
            });
        });
        res.json({ success: true });
...
async function adminBroadcast(req, res) {
    if (!db) return res.status(503).json({ error: 'DB not available' });
    try { const botResult = await broadcastToUsers(req.body.message); res.json({ success: true, bot: botResult }); }
    catch (err) { res.status(500).json({ error: 'Failed' }); }
}
...
async function adminListPromos(req, res) {
    ...
    try {
        ...
        res.json(snap.docs.map(d => ({ code: d.id, ...d.data(), createdAt: tsToMillis(d.data().createdAt) })));
    } catch (err) { res.status(500).json({ error: 'Failed' }); }
}
...
async function adminCreatePromo(req, res) {
    ...
    try {
        ...
        await db.collection('promoCodes').doc(code).set({
            code, coin, reward: Number(reward), usageLimit: Number(usageLimit), usedCount: 0, usedBy: [], enabled: true,
...
async function adminTogglePromo(req, res) {
    if (!db) return res.status(503).json({ error: 'DB not available' });
    try { await db.collection('promoCodes').doc(req.body.code).update({ enabled: !!req.body.enabled }); res.json({ success: true }); }
    catch (err) { res.status(500).json({ error: 'Failed' }); }
}

async function adminDeletePromo(req, res) {
    if (!db) return res.status(503).json({ error: 'DB not available' });
    try { await db.collection('promoCodes').doc(req.body.code).delete(); res.json({ success: true }); }
    catch (err) { res.status(500).json({ error: 'Failed' }); }
}

function validatePromoCode(code) { if (!code) return null; const t = String(code).trim().toUpperCase(); if (t.length < 3 || t.length > 32) return null; if (!/^[A-Z0-9_-]+$/.test(t)) return null; return t; }
...
async function redeemPromo(req, res) {
    ...
    try {
        ...
        await db.runTransaction(async (t) => {
            const u = await t.get(userRef); if (!u.exists) throw new Error('User not found');
            const fp = await t.get(promoRef); const fpd = fp.data();
            ...
            if (isCoin) {
                ...
                t.update(userRef, { balance: increment(reward), total_earned: increment(reward) });
            }
            t.update(promoRef, { usedCount: increment(1), usedBy: FieldValue.arrayUnion(userId) });
            t.set(db.collection('transactions').doc(), { user_id: userId, type: 'promo', amount: reward, currency: promo.coin, description: `Promo: ${code}`, metadata: { code }, timestamp: serverTimestamp() });
        });
...
async function getPromoHistory(req, res) {
    if (!db) return res.status(503).json({ error: 'DB not available' });
    try {
        const snap = await db.collection('transactions').where('user_id', '==', String(req.user.userId)).where('type', '==', 'promo').orderBy('timestamp', 'desc').limit(50).get();
        res.json({ history: snap.docs.map(d => ({ id: d.id, ...d.data(), timestamp: tsToMillis(d.data().timestamp) })) });
    } catch (err) { res.status(500).json({ error: 'Failed' }); }
}
...
app.get('/api/income-stats', jwtAuth, getIncomeStats);
...
app.use(express.static(publicPath, { maxAge: '1d', setHeaders: (res, p) => { if (p.endsWith('.html')) res.setHeader('Cache-Control', 'no-cache, no-store, must-revalidate'); } }));

app.get('*', (req, res) => {
    if (req.path.startsWith('/api/')) return res.status(404).json({ error: 'API endpoint not found' });
    res.sendFile(path.join(publicPath, 'index.html'), (err) => { if (err) res.status(404).send('App not found'); });
});

app.use((err, req, res, next) => { logger.error('Express error', { error: err.message, path: req.path }); res.status(500).json({ error: 'Internal server error' }); });
...
if (require.main === module) {
    ...
    if (process.env.NODE_ENV === 'production' && APP_URL) {
        ...
        setInterval(async () => { try { await fetch(`${APP_URL}/ping`, { cache: 'no-store' }); } catch (e) {} }, PING_INTERVAL_MS);
    }
    const shutdown = (signal) => { logger.info(`${signal} received, shutting down...`); server.close(() => process.exit(0)); setTimeout(() => process.exit(1), 10000).unref(); };
