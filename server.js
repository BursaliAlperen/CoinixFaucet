...
async function postback(req, res) {
  try {
    ...
    await db.collection('offerwall_completions').add({ user_id: userId, trans_id: transid, amount: amt, source: 'offerwall_native', timestamp: serverTimestamp() });
    res.send('ok');
